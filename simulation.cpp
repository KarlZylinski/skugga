...
static void create_scaled_box(World* world, Renderer* renderer, const Mesh& m, const Vector3& scale, const Vector3& pos, const Color& color)
{
...
static void create_world(World* world, Renderer* renderer)
{
    ...
    create_scaled_box(world, renderer, lm.mesh, {floor_width, floor_thickness, floor_depth}, {0, 0, 0}, color::random());
    create_scaled_box(world, renderer, lm.mesh, {pillar_width, floor_to_cieling, pillar_width}, {-1, (floor_thickness + floor_to_cieling) / 2, 1}, color::random());
    create_scaled_box(world, renderer, lm.mesh, {pillar_width, floor_to_cieling, pillar_width}, {-1, (floor_thickness + floor_to_cieling) / 2, -1}, color::random());
    create_scaled_box(world, renderer, lm.mesh, {floor_width, floor_thickness, floor_depth}, {0, floor_thickness + floor_to_cieling, 0}, color::random());
}
